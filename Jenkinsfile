pipeline {
    ...
    stages {
        stage('Create Job Parameters') {
            steps {
                script {
                    // Load the job.groovy to configure the parameters
                    def jobScript = load 'src/job.groovy'
                    jobScript // Executes the script that defines job parameters dynamically
                }
            }
        }
